...
class Machinery:
    ...
    def _initialize_check(self):
        """Run checks against virtualization software when a machine manager is initialized.
...
class LibVirtMachinery(Machinery):
    ...
    def _get_snapshot(self, label):
        ...
        def _extract_creation_time(node):
            """Extract creation time from a KVM vm config file.
...
class Signature:
    ...
    def on_signature(self, signature):
        ...
        Some signatures only take effect when one or more other signatures have matched as well.

        @param signature: The signature that just matched
        """

    def on_process(self, process):
        """Call on process change.
...
class BehaviorHandler:
    ...
    def parse(self, logpath):
        """Call after handles_path succeeded, should generate behavior events."""
        raise NotImplementedError
