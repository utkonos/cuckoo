...
def init_logger(name, level=None):
    ...
    if name == 'cuckoo.log':
        lh = logging.handlers.WatchedFileHandler(cwd('log', 'cuckoo.log'))
        lh.setFormatter(formatter)
        lh.setLevel(level)

    if name == 'cuckoo.json':
        j = JsonFormatter()
        lh = logging.handlers.WatchedFileHandler(cwd('log', 'cuckoo.json'))
        lh.setFormatter(j)
        lh.addFilter(j)
    ...
    if name.startswith('process-') and name.endswith('.json'):
        j = JsonFormatter()
        lh = logging.handlers.WatchedFileHandler(cwd('log', name))
        lh.setFormatter(j)
        lh.addFilter(j)
