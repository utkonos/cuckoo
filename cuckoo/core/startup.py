...
def check_configs():
    ...
    feedback_enabled = (config('cuckoo:feedback:enabled') or config('reporting:feedback:enabled'))
    if feedback_enabled:
        try:
...
def init_binaries():
    ...
    These include the Windows monitor etc.
    """
    def throw():
        raise CuckooStartupError(
...
def ensure_tmpdir():
    ...
    print(red('Cuckoo cannot read or write files into the temporary directory "%s",'
