...
def printf_format_for_struct(t, types):
    ...
    for (name, argtype) in type_description(t, types)['struct'].items():
        ...
        if printf_specifier is not None:
...
def pop_from_stack_section(args):
    ...
    parts = []
    for idx in range(len(args)):
        parts.append('self->arg%d = self->arguments_stack[self->deeplevel, "arg%d"];\tself->arguments_stack[self->deeplevel, "arg%d"] = 0;' % (idx, idx, idx))
