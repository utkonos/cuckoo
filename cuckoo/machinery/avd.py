...
class Avd(Machinery):
    ...
    def delete_old_emulator(self, label):
        """Delete any trace of an emulator that would have the same name as the one of the current emulator."""
    ...
    def replace_content_in_file(self, fileName, contentToReplace, replacementContent):
        """Replace the specified motif by a specified value in the specified file."""
    ...
    def check_adb_recognize_emulator(self, label):
        """Check that ADB recognizes the emulator. Return True if device is recognized by ADB, False otherwise."""
    ...
    def restart_adb_server(self):
        """Restart ADB server. This function is not used because we have to verify we don't have multiple devices."""
