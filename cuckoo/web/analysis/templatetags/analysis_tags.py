...
@register.filter
def volsort(l):
    """Sort baselined Volatility results. Results with `class_` set have a higher priority over the regular results."""
    if not l:
        return
...
@register.filter
def sigsort(l):
    """Sort signatures entries. Generic explanations come first, followed by IOCs, followed by API calls."""
    if not l:
        return
